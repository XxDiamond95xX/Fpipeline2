...
Réponds en JSON: {"felicitations":"100 mots chaleureux et personnels","recapitulatif":"200 mots qui récapitulent vraiment les apprentissages clés","plan_action_30_jours":["Semaine 1: action concrète et détaillée","Semaine 2: action","Semaine 3: action","Semaine 4: action"],"ressources_complementaires":["ressource gratuite pertinente 1","ressource 2","ressource 3"],"mot_de_fin":"120 mots inspirants et humains"}`,
    ...
    releaseWakeLock();
    setRunning(false);
    // Délai pour laisser React finir tous les setState avant de changer d'onglet
    setTimeout(() => setTab("deliverables"), 300);
  };
  ...
  // Fix: inclut l'agent content qui n'est pas dans AGENTS[]
  const ALL_IDS = [...AGENTS.map(a => a.id), "content"];
  const allDone = !running && ALL_IDS.every(id => agentStatus[id] === "done");
  const completedCount = AGENTS.filter(a => agentStatus[a.id] === "done").length;
  const progress = (completedCount / AGENTS.length) * 100;

  // Stats pré-calculées pour éviter les calculs lourds dans le rendu
  const ebookStats = (() => {
    try {
      const chapitres = agentResults.content?.chapitres || [];
      const totalMots = chapitres.reduce((acc, ch) =>
        acc + (ch.sections || []).reduce((a, s) => {
          const words = (s.contenu || "").split(/\s+/).filter(Boolean).length;
          return a + words;
        }, 0), 0);
      return {
        chapitres: chapitres.length,
        mots: totalMots,
        pages: Math.round(totalMots / 300)
      };
    } catch { return { chapitres: 0, mots: 0, pages: 0 }; }
  })();
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#070710", fontFamily: "'Space Mono', monospace", color: "#fff" }}>
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "24px 18px" }}>
        ...
        {/* DELIVERABLES TAB */}
        {tab === "deliverables" && allDone && (() => { try { return (
          <div style={{ animation: "fadeIn 0.4s ease" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 20 }}>
              {[
                { label: "Chapitres", val: ebookStats.chapitres || "—", color: "#a78bfa" },
                { label: "Mots rédigés", val: ebookStats.mots > 0 ? `~${ebookStats.mots.toLocaleString()}` : "—", color: "#00ffd5" },
                { label: "Pages PDF", val: ebookStats.pages > 0 ? `~${ebookStats.pages}` : "—", color: "#34d399" },
                { label: "Validation", val: agentResults.research?.validation === "GO" ? "✅ GO" : agentResults.research?.validation || "—", color: "#fb923c" }
              ].map((item, i) => (
        ...
        ); } catch(e) { return <div style={{color:"#ef4444",padding:20,fontFamily:"monospace",fontSize:12}}>⚠️ Erreur d'affichage: {e.message}<br/>Tes données sont sauvegardées. Essaie de télécharger le ZIP directement.</div>; } })()}
